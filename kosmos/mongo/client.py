...
def CollapseMongoURISecret(uri : str) -> str :
    logger.debug("Parsing Mongo URI %s", MaskMongoURI(uri))
    ...
    if ether.IsSecretSourceFormat(password) :
        logger.debug("MongoDB URI Password is from a secret source")
        password = ether.CollapseSecretSource(password)
    return f"{scheme}://{user}:{password}{host_and_path}"


def CollapseURIFor(purpose : PurposeAffinity) -> str :
