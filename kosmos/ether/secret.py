...
def IsSecretSourceFormat(s : str) -> bool:
    return s.startswith("__secret:") and s.endswith("__")

def CollapseSecretString(s : str) -> str:
    ...
    if len(parts) == 2:
        return SummonSecretManager().access_secret(parts[0], parts[1])

    return SummonSecretManager().access_secret(s, "latest")


def CollapseSecretSource(s : str) -> str:
    #check if the string is a holder string
    if not IsSecretSourceFormat(s) :
    ...
    #return the secret
    return SummonSecretManager().access_secret(parts[1], parts[2])
...
def SummonSecretManager() -> SecretManager :
    ...
    return GCPSecretManager(project_id=constants.ProjectID)
